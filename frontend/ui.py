...
# def get_data():
#     """
#     Makes a request to the specified backend endpoint to retrieve the data.

#     Returns:
#         dict: A dictionary containing the data results if the request was successful and the response format is json.
...
def get_plot_univarie(data, feature, id_client):
    ...
    g = sns.FacetGrid(data=data, hue="classes", height=5)
...
def run():
    ...
    def new_client():
        ...
        if btn_explain:
                ...
                with placeholder1.container(): 
                    ...
                    #shap.initjs()
                    shap_values_new = explainer.shap_values(df)
                    fig = shap.force_plot(explainer.expected_value, shap_values_new, matplotlib=True)
                    st.pyplot(fig)
    ...
    def previous_client():
        ...
        with placeholder3.container():
            ...
            if btn_explain:
                with st.spinner("Waiting for the explanation..."):
                    ...
                    with col2:
                        st.header("Local")
                        fig_l = shap.force_plot(explainer.expected_value, explainer.shap_values(data.iloc[int(id_client), :-1]), matplotlib=True)
                        st.pyplot(fig_l)
        
                    
       
        features_lst = data.columns.to_list()

        fig_col1, fig_col2 = st.columns(2)

        with fig_col1:
            #st.markdown("### Second Chart")
            feature_1 = st.selectbox('Select feature 1', features_lst[:-1])
            plt_feat1 = get_plot_univarie(data, feature_1, int(id_client))
            st.pyplot(plt_feat1)

            
        with fig_col2:
            feature_2 = st.selectbox('Select feature 2', features_lst[:-1])
        ...
        features = st.multiselect('What are your favorite colors',features_lst[:-1],features_lst[:-1])
        st.write(features[0])
        fig = px.scatter(data, x=features[0], y=features[1], color='classes', opacity=0.3)
